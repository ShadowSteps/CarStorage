...
namespace CarStorage\Data\Postgres\Sets;
...
use CarStorage\Data\Postgres\Entities\Crawlers;
use CarStorage\Data\Postgres\Entities\Jobs;
use CarStorage\Data\Postgres\Exceptions\NoJobsFoundException;
use CarStorage\Data\Postgres\Sets\Base\BaseSet;
...
class JobSet extends BaseSet implements IJobSet
{
    ...
    private function FillEntityFromData(JobData $data, Jobs &$entity){
         ...
         */
        $addedCrawler = $this->getManager()->find("CarStorage\Data\Postgres\Entities\Crawlers", $data->getAddedByCrawlerId());
        $entity->setCrawlerId($addedCrawler);
        if (!is_null($data->getDoneByCrawlerId())) {
            /**
             * @var $doneByCrawler Crawlers
             */
            $doneByCrawler = $this->getManager()->find("CarStorage\Data\Postgres\Entities\Crawlers", $data->getDoneByCrawlerId());
    ...
    public function Edit(string $id, JobData $data)
    {
        $entity = $this->getManager()
            ->find('CarStorage\Data\Postgres\Entities\Jobs', $id);
    ...
    public function GetNextFreeJob(): Job
    {
        ...
        $result = $this->getManager()
            ->getRepository('CarStorage\Data\Postgres\Entities\Jobs')
            ->matching($criteria);
        if ($result->isEmpty())
